...
import sokeriaaa.return0.applib.room.sq.SQSavedVariablesQueries
import sokeriaaa.return0.applib.room.table.SavedVariableTable

class SavedVariableDaoImpl(
    val queries: SQSavedVariablesQueries,
) : SavedVariableDao {
    ...
    override suspend fun queryAll(saveID: Int): List<SavedVariableTable> {
    ...
    override suspend fun insertOrUpdate(table: SavedVariableTable) {
        queries.insertOrUpdate(
            save_id = table.saveID.toLong(),
            saved_key = table.key,
            saved_value = table.value.toLong(),
        )
    }

    override suspend fun insertList(list: List<SavedVariableTable>) {
